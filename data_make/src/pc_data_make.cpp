 ...
 *       4. 添加对aloam算法的兼容性　20210811 主要在于点云话题， aloam直接订阅全局点云地图
 * 【订阅】
 ***************************************************************************/
...
gloabl_map_ptr_(new pcl::PointCloud<PointType>())
{
    ...
    nh.param<string>("/imu_potic", imu_topic_, "imu/data"); //for aloam  /laser_cloud_map


    laser_reg_pc_sub_ = nh.subscribe<sensor_msgs::PointCloud2>(pointcloud_topic_, 10, &DataMake::laser_reg_cloud_cb, this);
    laser_odom_sub_ = nh.subscribe<nav_msgs::Odometry>(laser_odom_topic_, 10, &DataMake::laser_odom_cb, this);
    ...
    laser_gloabl_map_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("laser_cloud_map_gloabl", 10);
...
void DataMake::laser_odom_cb(const nav_msgs::Odometry::ConstPtr &laser_odom_msg){
    ...
    if (pcl::geometry::squaredDistance(cloud_pose_3d_, cloud_pose_3d_last_) > sample_dis_) {
