...
from model_pts import PointNetCls
...
# dataloader
train_dataset = PointData(train_data_dir, label_file, 450)
train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)

test_dataset = PointData(test_data_dir, label_file, 450)
...
# 定义网络
net = PointNetCls(k=2).to(device=device)
...
for epoch in range(EPOCHS):
    ...
    for data in train_dataloader:
        ...
        correct = pred_choice.eq(label.data).cpu().sum()  # 判断和target是否匹配，并计算匹配的数量
        total_train_loss += loss
        scheduler.step()

    acc_train = correct.item() / float(BATCH_SIZE)
    ...
    with torch.no_grad():
        ...
        for data in test_dataloader:
            ...
            correct_test = pred_choice.eq(label.data).cpu().sum()
    ...
    print('[%d / %d] train loss: %f accuracy: %f == test loss: %f accuracy: %f ' %
          (epoch, EPOCHS, total_train_loss, acc_train, total_test_loss, correct_test.item() / float(BATCH_SIZE)))

    if (epoch % 200 == 0) and (epoch != 0):
        torch.save(net.state_dict(), os.path.join(save_dir,
                                                  "model_" + str(epoch)+"_"+str(total_train_loss.item())+".pth"))
        print("===========save: model_" + str(epoch)+"_"+str(total_train_loss.item()) + ".pth")
